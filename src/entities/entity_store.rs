...
impl EntityStore {
    ...
    pub fn create_entities_from_archetype(
        ...
        match self
            .allocator
            .try_allocate_fragmented(count, &mut instance_ranges)
        {
            Ok(_) => {}
            Err(needed) => {
                ...
                self.allocator
                    .allocate_fragmented(count, &mut instance_ranges);
            }
        }
        ...
        unsafe {
            ...
            for entity in entities {
                ...
                if (archetype != last_archetype) & !slots.is_empty() {
                    archetypes
                        .get_mut(last_archetype as usize)
                        .return_slots(slots);
                    slots.set_len(0);
                }
    ...
    fn reserve_entity_space(&mut self, size: usize) {
        ...
        self.instances
            .extend(repeat_with(Default::default).take(size));
    }
}
