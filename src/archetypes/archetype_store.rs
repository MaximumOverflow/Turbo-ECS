...
impl ArchetypeStore {
    ...
    pub fn create_archetype_with_capacity(
        ...
        for (query, results) in self.queries.iter_mut() {
            ...
            if !instance.matches_query(data.include()) { continue; }
            if  instance.matches_query(data.exclude()) { continue; }
    ...
    pub(crate) fn query(&mut self, query: EntityQuery) -> impl Iterator<Item=&mut ArchetypeInstance> {
        ...
        unsafe {
            ...
            self.queries.get(&query).unwrap().iter().map(move |i| {
                &mut *instances.add(*i)
            })
        }
    }
    ...
    fn init_query(&mut self, query: EntityQuery) {
        ...
        let indices = self.vec.iter().enumerate().filter_map(|(i, a)| {
            if !a.matches_query(data.include()) { return None; }
            if  a.matches_query(data.exclude()) { return None; }
            Some(i)
        });
